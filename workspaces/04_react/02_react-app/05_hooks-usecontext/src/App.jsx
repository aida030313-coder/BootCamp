...
/*
  1. React.createContext()
    ...
    4) createContext를 사용하면 여러 단계의 컴포넌트에 props를 일일이 전달하지 않고도(props drilling 회피), 
       원하는 값을 하위 컴포넌트에서 바로 사용할 수 있습니다.
      (props drilling: 최상위 컴포넌트부터 아래 컴포넌트로 props를 계속 전달하는 것을 의미합니다.)
    5) 컨텍스트 객체에는 Provider와 Consumer 컴포넌트가 포함되어 있습니다.
      (1) Provider 컴포넌트는 value prop을 통해 하위 컴포넌트에 값을 제공합니다. 
          하위 트리의 모든 컴포넌트에서 이 값을 사용할 수 있습니다.
      (2) 하위 컴포넌트에서는 React.useContext(Context) 또는 Context.Consumer를 통해 값을 읽을 수 있습니다. 
          함수형 컴포넌트에서는 React.useContext(Context) 사용이 더 간편합니다.
  ...
  3. React.useContext()
    ...
    4) Provider가 없으면, createContext에 지정한 defaultValue를 반환합니다. 
       대신 Provider가 value로 undefined를 명시적으로 전달하면 defaultValue는 반환되지 않고, undefined가 반환됩니다.
