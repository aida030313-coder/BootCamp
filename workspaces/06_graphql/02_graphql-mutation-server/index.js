...
// 샘플 데이터
const users = [
  { id: '1', username: 'Roger', age: 25},
  { id: '2', username: 'Mark', age: 30},
  { id: '3', username: 'Colin', age: 35}
];
...
const resolvers = {
  ...
  Mutation: {
    // args === { title: xx, content: xx, authorId: xx }
    createPost: (_, {title, content, authorId}) => {
        // 신규 게시글 생성
        const newPost = {
            id: String(posts.length + 101 + Math.random()),
            title,
            content: content || '(...)',
            authorId
        }
    ...
    deletePost: (_, { id }) => {
        ...
        // DB(배열)로부터 해당 id를 가진 게시글 객체의 인덱스 찾기
        const deleteIndex = posts.findIndex(post => post.id === id);

        // splice
        // [삭제된 요소 객체]
        const deletedPost = posts.splice(deleteIndex, 1) [0];
